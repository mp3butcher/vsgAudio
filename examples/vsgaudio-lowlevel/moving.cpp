...
#include <vsgAudio/AudioEnvironment.h> // necessary for 3D spatial audio update()
...
int main()
{
    try
    {
        ...
        vsg::ref_ptr<Source> beesound = Source::create("bee.wav");
        beesound->setGain(1);
        beesound->setLooping();
        ...
        while(angle<(M_PI*2.0*no_laps))
        {
            ...
            vsgAudio::AudioEnvironment::instance()->update(); // update 3D spatial state
        }
    }
    catch(Error e)
    {
