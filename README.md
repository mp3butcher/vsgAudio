# vsgAudio
vsgAudio is an VulkanSceneGraph nodekit that wraps OpenAL and FMOD to enable sound toolkits in VSG. The vsgAudio library is the result of collaborative work between Ames Lab, ARDEC, Chris Hanson (Alpha Pixel, LLC), and Paul Martz (Skew Matrix Software LLC). It's used as part of the VE-Suite project. vsgAudio is open source and available under the GNU LGPL v2.1 software license.

## Features
vsgAudio supports a compile-time selectable audio back end, which determines support for speaker configurations and specific audio file formats. Common speaker configurations (stereo, 5.1 surround) and file formats (.wav) are supported by both the OpenAL and FMOD back ends (within the limits of OpenAL and FMOD).

### Libraries
The vsgAudio library Includes the following features:
...
### Plugins
osgdb_vsgAudio Dot OSG file format support for the nodes and objects in the vsgAudio library.

### Examples
The project contains a small collection of example programs to demonstrate use of many of vsgAudio's features.
...
vsgAudio requires all of the following libraries:
-  VulkanSceneGraph (https://vsg-dev.github.io/vsg-dev.io/),
...
-  FMOD Ex (no dependencies) http://www.fmod.org/ (optional)

One of the two backends:

1. FMOD Backend Dependencies
...
The OpenAL++ abstraction layer is included with vsgAudio.
...
0_BUILD_EXAMPLES_vsgAudio builds the high-level OSG-integrated demo examples and is recommended.
0_BUILD_EXAMPLES_vsgAudio_LOWLEVEL builds the low-level non-OSG-dependent examples that mimic the
  original OpenAL++ low-level examples, but use the vsgAudio abstraction layer. Recommended.
0_BUILD_EXAMPLES_OPENALPP builds the original low-level non-OSG-dependent examples that use the
  OpenAL++ API directly and not the vsgAudio abstraction layer. These will only build if you compile
  with the OpenAL++ backend enabled. Recommended if you build with OpenAL++

### Build under Linux:

`sudo apt install libvorbis-dev libogg-dev libopenal-dev libalut-dev `
...
LibOGG and LibVorbis (including vorbisfile) must be built prior to building vsgAudio+openAL++.

FMOD:
No special steps.

Common:
Navigate to the vsgAudio folder within CMake and
configure all necessary paths, then generate build files for your development tools.


## Support
For support requests beyond issuing bugs through github issue tracker
...
VulkanSceneGraph Currently tested with 1.1.1 and later, should work with
...
### Testing:
  ...
  If you are using FMOD, or if that step passes, then test the vsgAudio low-level example/demo
  programs (example_vsgAudiolow_*). When those run ok, try the vsgAudio high-level OSG-integrated
  examples (example_vsgAudio_*).


### Licence:

 VulkanSceneGraph Audio Library, vsgAudio
 Copyright (C) 2025 Julien Valentin
 tribute to osgAudio Copyright (C) 2010 Chris Hanson, AlphaPixel, LLC
 tribute to osgAL, Copyright (C) 2004 VRlab, Umeå University
