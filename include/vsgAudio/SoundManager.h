...
#ifndef vsgAudio_SOUNDMANAGER_H
#define vsgAudio_SOUNDMANAGER_H 1
...
/*!
    ...
    */
class VSGAUDIO_EXPORT SoundManager
{
public:
    ...
    /*!
        ...
        */
    vsg::ref_ptr<Sample> getSample( const std::string& path, bool add_to_cache=true );
    ...
    /*!
        ...
        Otherwise it will be loaded from disk.
        */
    vsg::ref_ptr<Stream> getStream( const std::string& path, bool add_to_cache=true );
    ...
    /*!
        ...
        */
    vsg::ref_ptr<SoundState> findSoundState(const std::string& id);

    /*!
        Puts the source into the list of available sound sources
        */
    void releaseSource(vsg::ref_ptr<Source>&);
    ...
    vsgAudio::Listener *getListener();

    /// Return a pointer to the Sound environment
    vsgAudio::AudioEnvironment *getEnvironment();

    /*!
        Add a SoundState to the list of existing sound states
        */
    void addSoundState(vsg::ref_ptr<vsgAudio::SoundState> state) { m_sound_states.insert(state); }
    ...
    /*!
        ...
        */
    bool removeSoundState(vsg::ref_ptr<vsgAudio::SoundState> state);
    ...
    /*!
        ...
        */
    void setListenerDirection(const vsg::vec3& dir);

    const vsg::vec3& getListenerDirection() const { return m_listener_direction; }

    /// Set the transformation matrix for the listener
    void setListenerMatrix( const vsg::LookAt& lookat);//const vsg::mat4& matrix);

    /// Return the current listener matrix
    const vsg::mat4 &getListenerMatrix( ) const { return m_listener_matrix; }
    ...
    /*!
        ...
        */
    vsgAudio::Source *allocateSource(unsigned int priority, bool mutual_use=true) { return getSource(priority, mutual_use); }
...
private:
    vsgAudio::Source *getSource(unsigned int priority, bool registrate_as_active=true, int depth=0);
    ...
    class SoundStateFlyWeight : public vsg::Inherit<vsg::Object,SoundStateFlyWeight> {
    public:
    ...
    private:
        ...
        std::vector< vsg::ref_ptr<SoundState> > m_sound_states;
        std::stack< SoundState *> m_available_states;
    };

    vsg::ref_ptr<SoundStateFlyWeight> m_sound_state_FlyWeight;

    void resetSource(vsgAudio::Source *source);

    typedef std::map<std::string, vsg::ref_ptr<vsgAudio::Sample> > SampleMap;
    typedef SampleMap::iterator SampleMapIterator;
    typedef SampleMap::value_type SampleMapValType;

    SampleMap m_sample_cache;

    typedef std::map<std::string, vsg::ref_ptr<vsgAudio::FileStream> > StreamMap;
    typedef StreamMap::iterator StreamMapIterator;
    typedef StreamMap::value_type StreamMapValType;

    StreamMap m_stream_cache;

    vsg::ref_ptr<vsgAudio::Listener> m_listener;
    vsgAudio::AudioEnvironment* m_sound_environment;


    typedef std::vector<vsg::ref_ptr<vsgAudio::Source> > SourceVector;
    typedef std::vector<std::pair< unsigned int, vsg::ref_ptr<vsgAudio::Source> > > ActiveSourceVector;
    ...
    vsg::mat4 m_listener_matrix;

    SourceVector m_available_soundsources;

    typedef std::set<vsg::ref_ptr<SoundState> > SoundStateSet;
    ...
    vsg::clock::time_point m_timer;
    vsg::clock::time_point m_last_tick;
    bool m_first_run;
    bool m_clamp_velocity;
    float m_update_frequency;
    vsg::vec3 m_listener_direction;

};


inline vsgAudio::Listener *SoundManager::getListener()
{
    if (!m_initialized) {
...
inline vsgAudio::AudioEnvironment* SoundManager::getEnvironment()
{
    if (!m_initialized) {
...
#define g_SoundManager vsgAudio::SoundManager::instance()
#endif // vsgAudio_SOUNDMANAGER_H
