...
{
    vsg::ref_ptr<SoundState> state = 0;

    for(unsigned int i=0; i < no_states; i++) {
        state = SoundState::create("");
...
SoundManager::SoundManager()
    :
    m_sound_state_FlyWeight(nullptr),
    m_listener(nullptr),
...
{
    m_listener_direction = vsg::vec3(1,1,1);
}
...
void SoundManager::init(unsigned int num_soundsources, float sound_velocity, bool displayInitMsgs )
{
    ...
    {
        m_sound_environment = vsgAudio::AudioEnvironment::instance();
    ...
    if (!m_soundsources.size()) {
        ...
        catch(vsgAudio::NameError & e) {
            vsg::info( "SoundManager::init() NameError: " , e.what() );
        }
    }
    if (!m_soundsources.size())
...
SoundManager::~SoundManager()
{
    ...
    if (initialized()) {
        ...
        vsg::warn( "SoundManager::~SoundManager(): " , msg);
...
bool SoundManager::removeSoundState(vsg::ref_ptr<vsgAudio::SoundState> state)
{
    bool found = false;
    {
...
void SoundManager::releaseSource(vsg::ref_ptr<Source>& source)
{
    if (!m_initialized)
        return;
...
void SoundManager::processQueuedSoundStates()
{
    ...
    while(m_available_soundsources.size() && m_sound_state_queue.size()) {
        ...
        //vsg::notify(vsg::INFO) << "Adding m_active_sound_states size: " << m_active_sound_states.size() << std::endl;
        //vsg::notify(vsg::INFO) << "Available sources size: " << m_available_soundsources.size() << std::endl;
    }

    if( !m_available_soundsources.size() ) {
        vsg::warn("SoundManager::processQueuedSoundStates(): There are no more sources to be allocated." );
    }
}
...
vsg::ref_ptr<Sample> SoundManager::getSample( const std::string& path, bool add_to_cache )
{
    ...
    if (smi != m_sample_cache.end()) {
        found = true;
        vsg::info( "SoundManager::getSample(): Found " , path , " in cache" );
        sample = new Sample(*(smi->second));
    }
    else {
        vsg::info(  "SoundManager::getSample(): Cache miss for " , path , ". Loading from file..." );

        // Cache miss, load the file:
        /*std::string new_path = osgDB::findDataFile(path);
        if (new_path.empty()) {
            vsg::info(  "SoundManager::getSample(): Unable to find requested file: " , path );
            return 0;
        }*/
...
vsg::ref_ptr<Stream> SoundManager::getStream( const std::string& path, bool add_to_cache )
{
    vsg::ref_ptr<FileStream> stream=nullptr;
    StreamMap::iterator smi;
    smi = m_stream_cache.find(path);
    bool found=false;
    if (smi != m_stream_cache.end()) {
        found = true;
        vsg::info( path , " in cache" );
        stream = new FileStream(*(smi->second));
    }
    else {

        vsg::info( "SoundManager::getStream: Cache miss for " , path , ". Loading from file..." );
        ...
        try {
            ...
            /*std::string new_path = osgDB::findDataFile(path);
            if (new_path.empty()) {
                vsg::warn("SoundManager::getStream(): Unable to find requested file: " , path );

                return 0;
            }*/
            stream = new FileStream(path.c_str());
        }
        catch(vsgAudio::FileError) {
            vsg::ref_ptr<FileStream> s = stream;
        }
...
vsg::ref_ptr<SoundState> SoundManager::findSoundState(const std::string& id)
{
    for (SoundStateSet::iterator it = m_sound_states.begin();
         it != m_sound_states.end();
         ++it)
...
void SoundManager::setListenerDirection(const vsg::vec3& dir)
{
...
void SoundManager::setListenerMatrix( const vsg::LookAt& lookat)
{

    m_listener_matrix = lookat.transform();


    vsg::vec3 look_vector = vsg::vec3(lookat.center - lookat.eye);
    ...
    if (m_first_run) {
        m_first_run = false;
        m_last_tick = vsg::clock::now();//m_timer.tick();
        m_last_pos = lookat.eye;
    }
    else {
        auto curr_tick = vsg::clock::now();
        velocity = vsg::vec3(lookat.eye) - m_last_pos;
        m_last_pos = lookat.eye;

        float time=std::chrono::duration_cast<std::chrono::milliseconds>(curr_tick-m_last_tick).count()*0.001f;
        //double time =  m_timer.delta_s(m_last_tick, curr_tick);
        m_last_tick = curr_tick;
        velocity /= time;
    }

    const vsg::vec3& listener_direction = getListenerDirection();


    Listener *listener = getListener();
    listener->setPosition(lookat.eye[0], lookat.eye[1], lookat.eye[2]);
    listener->setOrientation(
        look_vector[0],
        look_vector[1],
        look_vector[2],
        lookat.up[0]*listener_direction[0],
        lookat.up[1]*listener_direction[1],
        lookat.up[2]*listener_direction[2]);

    if(getClampVelocity()) {
        float max_vel = getMaxVelocity();
        float len = vsg::length(velocity);
        if ( len > max_vel) {
            velocity = vsg::normalize(velocity);
            velocity *= max_vel;
        }
    }
