...
/**
* Used when translating the SourceState between vsgAudio and openAL++
*/
...
static vsgAudio::SourceState _vsgAudioSourceStateTranslate[] = {
    Initial,
    Playing,
    Paused,
    Stopped
...
const SoundData *Source::getSound() const {
    ...
    return(NULL); // we can't go from the openalpp::SoundData to a vsgAudio::SoundData right now
}
...
SourceState Source::getState() const {
    try {
        return _vsgAudioSourceStateTranslate[_openalppSource->getState()];
    }
...
void Source::setReverbScale(float scale) noexcept(false) {
    try {
...
void Source::setReverbDelay(float delay) noexcept(false) {
    try {
...
float Source::getReverbScale() noexcept(false) {
    try {
...
float Source::getReverbDelay() noexcept(false) {
    try {
