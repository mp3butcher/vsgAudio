...
#include <vsgAudio/SoundDefaults.h>
...
Source::Source( const Source &source) {
    ...
    FMOD_RESULT createResult = vsgAudio::AudioEnvironment::instance()->getSystem()->playSound(FMOD_CHANNEL_FREE, _Sound->getInternalSound(), true, &_FMODChannel);
    initPitchSupport();
...
void Source::play() {
    ...
    // not yet playing
    FMOD_RESULT createResult = vsgAudio::AudioEnvironment::instance()->getSystem()->playSound(FMOD_CHANNEL_FREE, _Sound->getInternalSound(), false, &_FMODChannel);
    initPitchSupport();
}
...
bool Source::isStreaming() {
    vsgAudio::Stream *testStream = dynamic_cast<vsgAudio::Stream *>(_Sound.get());
