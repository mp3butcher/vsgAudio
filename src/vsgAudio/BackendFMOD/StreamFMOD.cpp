...
#include <vsgAudio/BackendFMOD/AudioEnvironmentFMOD.h>
...
void Stream::createStreamFromFilename(const std::string& filename ) noexcept(false)
{
    _FMODSound = NULL;
    _filename = filename;

    FMOD_RESULT createResult;
    createResult = vsgAudio::AudioEnvironment::instance()->
                   getSystem()->createSound(filename.c_str(),
                                     FMOD_3D | vsgAudio::SoundManager::instance()->getEnvironment()->getInternalDistanceModel(),
                                     0, &_FMODSound);
