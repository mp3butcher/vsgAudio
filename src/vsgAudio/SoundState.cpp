...
SoundState::SoundState(const SoundState& state, const vsg::CopyOp& copyop) : vsg::Inherit<vsg::Object, SoundState>(state, copyop)
{
...
void SoundState::releaseSource()
{
    ...
    if (m_source.valid())
        m_sound_manager->releaseSource(m_source);
    m_source = 0;
}
...
bool SoundState::isActive()
{
    ...
    if (!m_source.valid())
        ...
        f= (m_source->getState() == vsgAudio::Playing);
...
void SoundState::apply()
{
    ...
    if (!m_sample.valid() && !m_stream.valid()) {
        //vsg::notify(vsg::WARN) << "SoundState::apply(): No sample or stream assigned to SoundState" << std::endl;
        return;
    }
