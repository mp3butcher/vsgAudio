...
GroupSource::GroupSource(float x,float y,float z) noexcept(false)
...
void GroupSource::play() noexcept(false) {
    try {
        if(!mixed_)
            mixSources();
...
ALuint GroupSource::includeSource(vsg::ref_ptr<Source> source) noexcept(false) {
...
void GroupSource::excludeSource(const Source &source) noexcept(false) {
    ALuint sourcename=source.getAlSource();
    excludeSource(sourcename);
}

void GroupSource::excludeSource(ALuint sourcename) noexcept(false) {
