...
void SourceBase::init() noexcept(false) {
...
SourceBase::SourceBase() noexcept(false) : PositionedObject() {
    init();
}

SourceBase::SourceBase(float x,float y,float z)
    noexcept(false) : PositionedObject() {
...
void SourceBase::setReverbScale(float scale) noexcept(false) {
    if(reverbinitiated_) {
...
void SourceBase::setReverbDelay(float delay) noexcept(false){
    if(reverbinitiated_) {
...
float SourceBase::getReverbScale() noexcept(false) {
    if(!reverbinitiated_)
        throw InitError("Reverb not initialized");
    return reverbscale_;
}

float SourceBase::getReverbDelay() noexcept(false) {
    if(!reverbinitiated_)
        throw InitError("Reverb not initialized");
    return reverbdelay_;
}

ALuint SourceBase::link(const SourceBase *source) noexcept(false) {
...
void SourceBase::unlink(const SourceBase *source) noexcept(false) {
...
void SourceBase::unlink(const ALuint name) noexcept(false) {
    if(name==sourcename_)
        throw(NameError("Can't unlink a source from itself!"));
    for(unsigned int i=0;i<nlinkedsources_;i++)
