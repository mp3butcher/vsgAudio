...
FileStreamUpdater::FileStreamUpdater(
    ...
    : vsg::Inherit<StreamUpdater, FileStreamUpdater>(buffer1,buffer2,format,frequency),
    ...
    seekTime_(0)
{
   ...
   //OpenThreads:: setCancelModeAsynchronous();
...
{
    std::lock_guard<std::recursive_mutex> lock(runmutex_);
